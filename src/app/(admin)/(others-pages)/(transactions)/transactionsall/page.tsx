...
export default function TransactionsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch earnings data
        const earningsResponse = await axios.get('https://shreejighutargo21.onrender.com/api/vendors/vendor/earnings', {
          headers: { Authorization: `Bearer ${token}` },
        });
  
        // Fetch wallet data
        const walletResponse = await axios.get('https://shreejighutargo21.onrender.com/api/vendors/api/vendor/wallet', {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSubmitWithdrawal = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        'https://shreejighutargo21.onrender.com/api/vendors/withdrawal/requests',
        formData,
        {
          headers: { 
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          {showTransactionButton && (
            <motion.button
              ...
              className={`font-semibold py-2 px-9 rounded-lg shadow-lg transition-all duration-200 ${
                isButtonDisabled
